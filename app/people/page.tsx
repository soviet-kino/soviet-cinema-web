  ...
  return (
    <section className="space-y-6">
      <header className="flex items-baseline justify-between gap-4 flex-wrap">
        ...
        <p className="text-sm text-light/60">{people.length} в базе</p>
      </header>
      ...
      <ul className="grid sm:grid-cols-2 gap-x-6 divide-y divide-light/10 sm:divide-y-0">
        {people.map((p) => (
          <li
            key={p.id}
          ...
          >
            <Link href={`/people/${p.id}`} className="hover:underline">
              ...
              {p.roles.length > 0 && (
                <span className="text-light/60 text-sm ml-2">
                  {p.roles.map((r, i) => (
                    <span key={r}>
                      {i > 0 && ", "}
