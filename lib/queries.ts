...
export function staticPersonIds(): string[] {
  ...
  for (const r of peopleRows) {
    ...
    // Актёры без фото — на 404 независимо от числа ролей. Известные
    // многоснимавшиеся актёры почти все уже с фото из Wikidata, так
    // что теряем в основном эпизодические заглушки. Они остаются в
    // списках и фильмографиях, но клик → 404.
    //
    // ВАЖНО: ветку «cast >= 8 без фото» убрали 2026-05-31 — после
    // прогона enrich-people (+4468 портретов) суммарный бюджет файлов
    // перевалил за 20k и деплой падал. См. project-state memory.
    const include =
      !!p.image_commons ||
      (c?.crew ?? 0) > 0;
