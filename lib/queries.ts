...
export function staticPersonIds(): string[] {
  ...
  for (const r of peopleRows) {
    ...
    //   - всех с фото (image_commons);
    //   - crew от 2+ фильмов (устойчивые постановщики);
    //   - crew одного фильма, НО с датой рождения — идентифицированная
    //     персона, а не голая заглушка.
    // Актёры без фото и голые одно-кредитные заглышки без даты рождения
    // → 404. Остаются в списках и фильмографиях, клик → 404.
    //
    // ВАЖНО: критерий ужесточён 2026-06-22 после импорта мультипликации
    // (+476 фильмов, +98 crew-заглушек). Прежний «crew>0» давал ~12k
    // людей и суммарно ~20.3k файлов — пробивал лимит. Текущий ~10.6k.
    // ВАЖНО: ветку «cast >= 8 без фото» убрали 2026-05-31 (см. memory).
    const crew = c?.crew ?? 0;
    const include =
      !!p.image_commons ||
      crew >= 2 ||
